...
fn load_cubemap() -> Result<()> {
    ...
    if let Format::Uncompressed { pitch, color_format, alpha_format } = format {
        assert_eq!(pitch, 3, "Format should be 3-byte pitch");
        assert_eq!(color_format, ColorFormat::RGB {
            r_mask: 0xFF,
            g_mask: 0xFF00,
            b_mask: 0xFF0000,
            srgb: false,
    ...
    for (face, surface) in texture.iter_faces() {
        let surface = surface.try_into_surface().expect("Cubemap faces should be surface primitives");
        ...
        assert_eq!(buffer[..3], [0xFF, 0x00, 0xFF], "First pixel is not magenta");
        assert_eq!(buffer[buffer.len() - 3..], [0xFF, 0x00, 0xFF], "Last pixel is not magenta");
    }
